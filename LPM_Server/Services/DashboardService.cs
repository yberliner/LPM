...
public class DashboardService
{
    ...
    /// Returns defaults for a specific auditor+PC pair from their most recent non-free
    /// approved session. Returns (0, 0) when this auditor has never had an approved session
    /// for this PC — the UI uses (0, 0) as the "brand-new PC" signal.
        ...
        if (isSolo)
        {
            ...
            cmd.CommandText = $@"
                SELECT s.SessionId, s.PcId,
                       ...
                       s.SessionDate,
                       COALESCE(s.LengthSeconds,0) + COALESCE(s.AdminSeconds,0) AS TotalSec,
                       CASE WHEN cr.CsReviewId IS NULL THEN 'Waiting' ELSE COALESCE(cr.Status, 'Done') END AS CsStatus,
                       TRIM(pc.FirstName || ' ' || COALESCE(NULLIF(pc.LastName,''), '')) AS CsName,
                       COALESCE(s.CreatedAt, '') AS CreatedAt,
                       1 AS IsApproved,
